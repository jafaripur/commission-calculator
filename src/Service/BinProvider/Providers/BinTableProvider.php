 ...
 * https://bintable.com.
 */
final class BinTableProvider implements BinProviderInterface
{
    private const HTT_CONTENT_TYPE_ACCEPT = 'application/json';
    private const CACHE_KEY = 'bin_table_provider';
    ...
    {
        $data = $this->cacheEnabled ? $this->cache->get($this->cacheKey.$identity, function (ItemInterface $item) use ($identity) {
            return $this->makeRequest($identity, $item);
        }) : $this->makeRequest($identity);

        return (new BinInformation())
            ->setCountryCode($data['data']['country']['code'] ?? '')
            ->setCountryName($data['data']['country']['name'] ?? '');
    }

    private function makeRequest(string $identity, ?ItemInterface $item = null): array
    {
        $result = $this->http->get(sprintf($this->endpoint, $identity, $this->token), [
